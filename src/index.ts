    ...
    try {
        ...
        switch (method) {
            ...
            case 'update_product_meta':
                ...
                let metaData: WooCommerceMeta[] = product.meta_data || [];

                // Look for existing meta with the same key
                const existingMetaIndex = metaData.findIndex((meta: WooCommerceMeta) => meta.key === params.metaKey);
            ...
            case 'delete_product_meta':
                ...
                let currentMetaData: WooCommerceMeta[] = productForDelete.meta_data || [];

                // Filter out the meta key to delete
                const updatedMetaData = currentMetaData.filter((meta: WooCommerceMeta) => meta.key !== params.metaKey);
            ...
            case 'update_order_meta':
                ...
                let orderMeta: WooCommerceMeta[] = order.meta_data || [];

                // Look for existing meta with the same key
                const existingOrderMetaIndex = orderMeta.findIndex((meta: WooCommerceMeta) => meta.key === params.metaKey);
            ...
            case 'delete_order_meta':
                ...
                let currentOrderMeta: WooCommerceMeta[] = orderForDelete.meta_data || [];

                // Filter out the meta key to delete
                const updatedOrderMeta = currentOrderMeta.filter((meta: WooCommerceMeta) => meta.key !== params.metaKey);
            ...
            case 'update_customer_meta':
                ...
                let customerMeta: WooCommerceMeta[] = customer.meta_data || [];

                // Look for existing meta with the same key
                const existingCustomerMetaIndex = customerMeta.findIndex((meta: WooCommerceMeta) => meta.key === params.metaKey);
            ...
            case 'delete_customer_meta':
                ...
                let currentCustomerMeta: WooCommerceMeta[] = customerForDelete.meta_data || [];

                // Filter out the meta key to delete
                const updatedCustomerMeta = currentCustomerMeta.filter((meta: WooCommerceMeta) => meta.key !== params.metaKey);
